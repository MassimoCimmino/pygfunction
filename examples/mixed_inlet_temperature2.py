...
def main():
    ...
    H = np.array([100, 125, 150, 125, 100])           # Borehole length (m)
    ...
    ts = np.mean(H)**2/(9.*alpha)            # Bore field characteristic time
    cells_per_level = 6
    time = []
    _width = []
    i = 0
    ...
    # Field of N_1x1 boreholes
    N_1 = 5
    boreField = []
    for i in range(N_1):
        borehole = gt.boreholes.Borehole(x=i*B, y=0, H=H[i], D=D, r_b=r_b)
...
#            nSegments=nSegments, disp=True)
    ...
    m_flow = m_flow*np.ones(nBoreholes)
...
#    ax1.set_ylim([0., 50.])
    ...
    for i in range(nBoreholes):
        # Fluid temperatures
        nz = 30
        z = np.linspace(0., H[i], num=nz)
        HSeg = H[i]/nSegments
        zb = np.array([0.5*HSeg+j*HSeg for j in range(nSegments)])
