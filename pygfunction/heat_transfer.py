    ...
    if time is np.inf:
        ...
        if np.any(borefield_j.is_tilted) or np.any(borefield_i.is_tilted):
            ...
            h = _finite_line_source_steady_state(
                borefield_j, borefield_i, reaSource=reaSource,
                imgSource=imgSource, outer=outer)
    elif approximation:
        ...
        if np.any(borefield_j.is_tilted) or np.any(borefield_i.is_tilted):
            ...
            # Vertical boreholes
            h = _finite_line_source_approximation(
...
def finite_line_source_vertical(
    ...
    if time is np.inf:
        # Steady-state finite line source solution
        h = _finite_line_source_steady_state(
            borefield_j, borefield_i, distances=distances, outer=outer,
            reaSource=reaSource, imgSource=imgSource)
    elif approximation:
        # Approximation of the finite line source solution
        h = _finite_line_source_approximation(
            time, alpha, borefield_j, borefield_i, distances=distances,
            outer=outer, reaSource=reaSource, imgSource=imgSource, N=N)
    else:
        # Integrand of the finite line source solution
        f = _finite_line_source_integrand(
            borefield_j, borefield_i, distances=distances, outer=outer,
...
def _finite_line_source_approximation(
        time: npt.ArrayLike,
        alpha: float,
...
def _finite_line_source_integrand(
        borefield_j: Borefield,
        borefield_i: Borefield,
        distances: Union[None, npt.ArrayLike] = None,
        reaSource: bool = True,
    ...
    if distances is None:
        ...
        if isinstance(dis, list):
            dis = np.atleast_1d(dis)
        f = lambda s: s**-2 * np.multiply.outer((erfint(q * s).T @ p).T, np.exp(-dis**2 * s**2))
    return f
...
def _finite_line_source_steady_state(
        borefield_j: Borefield,
        borefield_i: Borefield,
        distances: Union[None, npt.ArrayLike] = None,
        reaSource: bool = True,
    ...
    if distances is None:
        ...
        if isinstance(dis, np.ndarray):
            q = q[..., np.newaxis]
    h = 0.5 * (((q * np.log(q + q2_plus_dis2) - q2_plus_dis2).T @ p) / H_i).T
    return h
