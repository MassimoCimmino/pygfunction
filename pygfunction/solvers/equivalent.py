...
class Equivalent(_BaseSolver):
    ...
    def thermal_response_factors(
        ...
        for pairs in self.borehole_to_borehole:
            ...
            N2 = np.array(
                [self.borefield[j].nBoreholes for (i, j) in pairs])
            # Evaluate FLS at all time steps
            borefield_1 = Borefield(H1, D1, self.borefield.r_b[i], 0., 0.)
            borefield_2 = Borefield(H2, D2, self.borefield.r_b[j], 0., 0.)
            h = finite_line_source_vertical(
                time, alpha, borefield_1, borefield_2, distances=dis,
                weights=(wDis/N2).T, outer=False)
            # Broadcast values to h_ij matrix
            for k, (i, j) in enumerate(pairs):
                i_segment = i0[i] + i_pair
                j_segment = i0[j] + j_pair
                h_ij[j_segment, i_segment, 1:] = h[k_pair, k, :]
                if not i == j:
                    h_ij[i_segment, j_segment, 1:] = (
                        h[k_pair, k, :]
                        * segment_lengths[j_segment, np.newaxis]
                        / segment_lengths[i_segment, np.newaxis]
                        )
    ...
    def find_groups(self, tol: float = 1e-6) -> int:
        ...
        if self.nBoreholes > 1:
            ...
            data = np.sum(
                finite_line_source(
                    np.inf, 1., self.borefield, self.borefield
                    ), axis=1
                ).reshape(-1,1)
